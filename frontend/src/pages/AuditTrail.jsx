...
export default function AuditTrail({ caseId, refreshKey }) {
  const [data, setData] = useState(null)
  const [error, setError] = useState(null)

  useEffect(() => {
    api.audit(caseId).then(setData).catch((e) => setError(e.message))
  }, [caseId, refreshKey])
      ...
      {data && (
        <>
          ...
          {data.entries.length === 0 && (
            <p className="empty-state" style={{ marginTop: 12 }}>No actions logged yet for this case. Run ingestion first.</p>
          )}
